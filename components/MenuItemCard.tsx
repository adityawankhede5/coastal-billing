...
export default function MenuItemCard({ item, quantity = 0, query, handleUpdateCart }: { item: MENU_ITEM, quantity: number, query: string, handleUpdateCart: (itemId: string, quantity: number) => void }) {
  ...
  const onPlusMinusClick = (event: React.MouseEvent<HTMLSpanElement>, quantity: number = 1) => {
  ...
  return (
    <div className={`relative bg-white rounded-xl grid grid-rows-[auto_1fr_auto] shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-300 w-full min-h-56 max-h-56 p-4 border ${quantity > 0 ? "border-indigo-400" : "border-indigo-50"}`} onClick={(e) => onPlusMinusClick(e, 1)}>
      ...
      <div className="flex items-center gap-1 h-8">
        <span style={{ opacity: quantity <= 0 ? 0.3 : 1 }} className="bg-gray-900 rounded-full w-8 h-8 text-white flex items-center justify-center" onClick={(e) => onPlusMinusClick(e, -1)}><MinusIcon /></span>
        <span className="text-lg font-bold text-indigo-600 text-center w-6">
          {quantity}
        </span>
        <span className="bg-gray-900 rounded-full w-8 h-8 text-white flex items-center justify-center" onClick={(e) => onPlusMinusClick(e, 1)}><PlusIcon /></span>
